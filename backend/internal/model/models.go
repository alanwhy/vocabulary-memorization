// Package model 定义应用层与存储层共享的领域数据和纯业务规则。
//
// 调用链固定为 app/storage -> model；本包不依赖 HTTP、数据库或其他内部包，
// 让两侧共享模型和算法时不会形成反向依赖。分页信封等 HTTP DTO 不属于领域模型，
// 因此仍由 app 包维护。
package model
...
type Word struct {
	...
	// 释义（后台查词会整段覆盖），这里是用户手标、独立读写，读侧由存储层归一为非 nil 空数组。
...
// DictionaryEntry 是管理员词库列表的一条记录，由存储层查询并交给应用层响应。
