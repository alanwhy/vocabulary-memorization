...
- 查词是异步进行的：录入单词时立即入库并返回，不会等接口响应；释义会在后台查到后自动写回，前端此时显示「查词中...」，写回后自动刷新展示，不会卡顿输入。
...
4. 部署完成后，浏览器访问 `http://101.42.45.60:39100`，用 `.env` 里的 `ADMIN_USERNAME`/`ADMIN_PASSWORD` 登录（超管账号），手机和电脑用同一个账号登录即可看到同一份数据。

`.env` 现在已经加入 `.gitignore`，不会再被提交到仓库里（里面存的是数据库密码、超管密码和 DeepSeek API Key），换新机器部署时记得把这个文件也一起拷过去，不然会重新生成一套新的密码/账号。

### 端口没打开怎么办

`docker compose up` 成功不代表外网能访问到，39100 端口需要在云服务商控制台的**安全组**里放行（这是最常见的坑），如果服务器本机还开了 `ufw` 之类的防火墙，也要 `ufw allow 39100`。
