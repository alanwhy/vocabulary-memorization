...
- 🃏 **间隔重复闪卡**：简化版 SM-2 排期，四档自评（记住 / 困难 / 模糊 / 不认识），反复忘的词优先出现，点「记住」自动归档。
- 🔊 **语音朗读**：豆包（火山引擎）语音合成，查词成功后后台预生成发音，单词卡和闪卡正面一键点读；闪卡展示新卡片时自动朗读一次。
- 📊 **统计可视化**：ECharts 绘制背诵趋势、次数分布、近 7 天词云、开头字母统计，数据全部由后端 SQL 聚合。
- 👥 **多用户 + 后台**：账号之间数据隔离；超管可增删用户、重置密码、禁用账号、管理全局词库，以及查词、语音和次数渐变配置。
...
- 到期单词进入闪卡队列，每组 30 张，本地翻卡自评，自评档位有四档：

| 档位 | 排期 | 结果 |
|---|---|---|
| 记住（good） | 间隔按难度系数倍增 | 直接归档 |
| 困难（hard） | 间隔 ×1.2，难度 −0.15 | 保持未归档，稍后重现 |
| 模糊（fuzzy） | 首次 1 天；之后间隔 ×1.1 向上取整，难度 −0.10 | 保持未归档，看例句后认出时使用 |
...
- 管理员可设置次数渐变的起始色、终止色和分段数；例句中已出现于系统词库的词按相同次数渐变显示，近义词、反义词、形近词命中词库时以主题色提示。
...
- 超管在 `/admin` 可：新增用户（可设超管）、查看创建/最后登录时间与录入数、重置密码、禁用/启用、删除（级联清理单词）；管理全局词库（过滤、删除、批量删除、对「暂无释义」词条重新查询、导出 CSV）；修改 DeepSeek 查词、豆包语音和次数渐变配置。
